#!/usr/bin/env python3
"""Run a simple YOLOv12 prediction from a local sunsmarterjie/yolov12 clone."""
...
DEFAULT_INPUT_DIR = QUESTION_DIR / "inputs" / "yolov12"
DEFAULT_OUTPUT_DIR = QUESTION_DIR / "outputs" / "yolov12"
DEFAULT_PROJECT_DIR = QUESTION_DIR / "runs" / "detect"
IMAGE_SUFFIXES = {".jpg", ".jpeg", ".png", ".bmp", ".webp"}
ANSI_ESCAPE_RE = re.compile(r"\x1b\[[0-9;]*m")

# YOLOv12 uses the same wrapper shape as YOLO11 so the comparison stays focused
# on the model/repo difference rather than on different local tooling.
INLINE_SCRIPT = r'''
import argparse
...
def resolve_python(repo_dir: Path, requested: str | None) -> str:
    ...
    # installed by setup_yolov12_osc.sh for this question.
    if requested:
        return requested
...
def copy_saved_images(
    ...
    # YOLOv12 saves into runs/detect/<name>/ first; this helper moves the curated
    # final artifacts into the local outputs/ folder used in the README.
    available = sorted(
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Simple YOLOv12 demo runner (GitHub source install)."
    )
    parser.add_argument(
        "--repo-dir",
        default=str(DEFAULT_REPO_DIR),
        help=(
            "Path to yolov12 repo clone. Defaults to <repo>/external/yolov12."
        ),
    )
    parser.add_argument("--python", default=None, help="Python interpreter to run inference.")
    parser.add_argument("--model", default="yolov12n.pt", help="Model checkpoint name/path.")
    parser.add_argument(
        "--source",
    ...
    parser.add_argument("--name", default="yolov12_demo", help="YOLO run name.")
...
def main() -> int:
    ...
    print(
        f"Processing YOLOv12 on {describe_source(resolved_source, source_path, len(output_names))}",
        flush=True,
    )
    ...
    # The raw run artifacts stay under project/name; this wrapper reports that
    ...
    for copied_path in copied_paths:
        print(f"Saved YOLOv12 annotated image to: {copied_path}")
    return 0
